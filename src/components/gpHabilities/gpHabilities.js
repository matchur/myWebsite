...
const GpHabilities = () => {
  ...
  const buttonLabel = winStatus ? 'Desligar' : 'Ligar';

  const panelStyle = useMemo(
    () => ({
      backgroundImage: winStatus ? "url('./imgs/95wallpaper.jpg')" : 'none',
    }),
    [winStatus],
  );
  ...
  const additionalTextStyle = useMemo(
    () => ({
      display: showAdditionalText ? 'flex' : 'none',
      marginBottom: winStatus ? '0px' : '80px',
    }),
    [showAdditionalText, winStatus],
  );
  ...
  return (
    <div className="Group-principalHab">
      <div id="Group-habilities" className="Group-habilities" style={panelStyle}>
        ...
        <div id="additional-text" style={additionalTextStyle} onClick={togglePower}>
          <div id="additional-textContent">
            <img
              id="imgDesligar"
              className="imgDesligar"
              style={{ filter: winStatus ? 'hue-rotate(0deg)' : 'hue-rotate(120deg)' }}
              src="./imgs/winXP/arreter.png"
              alt="Ícone do Windows Desligar"
            />
            <span id="text-button">{buttonLabel}</span>
          </div>
        </div>

        {winStatus && (
          <div id="Group-toolbar" className="Group-toolbar">
            <div className="start-button" onClick={toggleText}>
              <img src="./imgs/winXP/icone-do-matchur.png" alt="Ícone do Windows Iniciar" />
              <span>
                <b>Iniciar</b>
              </span>
            </div>
